...
function bdh_intraday_bars(session::Session, security::AbstractString, event_type::AbstractString, date_start::T1, date_end::T2;
    interval::Integer=UInt32(1), # defaults to 1 minute intervals
    ...
    for_each_response_message_element(queue, corr_id, timeout_milliseconds=timeout_milliseconds, verbose=verbose) do element
        @assert has_name(element, "IntradayBarResponse")
        ...
        @assert has_name(response_element, "barData")
        bar_data_array = response_element["barTickData"]
        @assert isa(bar_data_array, Element{true,BLPAPI_DATATYPE_SEQUENCE})
        push_named_tuples!(result, bar_data_array)
    end
